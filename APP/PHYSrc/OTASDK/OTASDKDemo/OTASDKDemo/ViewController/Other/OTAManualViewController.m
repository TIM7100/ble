...
@interface OTAManualViewController ()<JCBluetoothManagerDelegate,MBProgressHUDDelegate>{
    NSTimer *_timer;
    NSInteger _count;
}

@property (weak, nonatomic) IBOutlet UIButton *startOTAbtn;
@property (weak, nonatomic) IBOutlet UIButton *connectOTABtn;
...
@implementation OTAManualViewController
...
- (void)setUpBluetooth {
    ...
    [_bluetoothManager setUpdateMode:false];
    if ([_bluetoothManager.currentPeripheral.name containsString:@"PPlusOTA"]) {
        self.startOTAbtn.enabled = NO;
        [self.startOTAbtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
        self.connectOTABtn.enabled = NO;
        [self.connectOTABtn setTitleColor:[UIColor lightGrayColor] forState:UIControlStateNormal];
    }
}

- (IBAction)connectOrdinaryBle:(UIButton *)sender {

}

- (IBAction)startOTA:(UIButton *)sender {
    NSLog(@"设置设备ota状态");
    [_bluetoothManager startOTA];
}

- (IBAction)connectOTAAction:(UIButton *)sender {
...
- (IBAction)updateOTA:(UIButton *)sender {
...
-(void)updateOTAProgressDataback:(JCBluetoothManager *)manager feedBackInfo:(float)progressValue {
    ...
    self.progressValueLabel.text = [NSString stringWithFormat:@"%.2f",progressValue];
    self.valueProgressView.progress = progressValue/100;
}

-(void)updateOTAProgressDataback:(JCBluetoothManager *)manager isComplete:(BOOL)isComplete {
    if (isComplete) {
        dispatch_async(dispatch_get_main_queue(), ^{
            self.progressValueLabel.text = [NSString stringWithFormat:@"%.2f",100.00];
            NSLog(@"self.progressValueLabel.text:%@",self.progressValueLabel.text);
            self.valueProgressView.progress = 1;
        });
    }
}
...
- (void)bluetoothManager:(JCBluetoothManager *)manager didSucceedConectPeripheral:(CBPeripheral *)peripheral {
    ...
    if ([self.OTAOrAPPType isEqualToString:@"OTA"]) {
        NSString *tip = [NSString stringWithFormat:@"连接成功，请进行下一步"];
        [self hudSetting:tip];
    }
