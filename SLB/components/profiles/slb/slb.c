...
//=============================================================================
// 函数名称: slb_boot_load_exch_zone
// 功能描述: Bootloader 启动时检查交换区，判断是否有新固件需要升级，如果有就安装它
// 返回值  : PPlus_SUCCESS      - 升级完成，后续跳应用
//          PPlus_ERR_xxx       - 升级失败，直接跳应用
//=============================================================================
int slb_boot_load_exch_zone(void)
{
    int ret = PPlus_SUCCESS;
    uint32_t part_num = 0;         // 交换区中的分区总数
    uint8_t read_tmp_data[32];     // 临时读取缓冲区
    uint32_t part_info[4];         // 存储单个分区信息 (flash_addr, run_addr, size, checksum)

    //-------------------------------------------------------------------------
    // 第一步: 检查升级标志 "OTAF"
    //  如果交换区开头不是这个标记，说明没有待升级的固件，直接返回
    //-------------------------------------------------------------------------
    read_tmp_data[4] = 0;  // 确保字符串结束
    slb_spif_read(SLB_FLASH_FW_BASE, read_tmp_data, 4);  // 读取交换区开头4字节

    // 比较是否为 "OTAF" 魔术字
    if(strcmp((const char*)read_tmp_data, tag_ota_fw) != 0)
    {
        // 没有有效的升级标记，返回错误，Bootloader 会直接跳转到现有应用
    ...
    // 升级条件检查:
    //  - 至少 2 个分区 (分区0是固件信息，至少要有一个实际数据分区)
    //  - 不超过最大支持分区数 32
    if(part_num < 2 || part_num > OTAF_PARTITION_NUM_MAX)
    {
        SLB_DELETE_EXCHANGE_ZONE();  // 擦除升级标记，避免下次启动还尝试升级
        return PPlus_ERR_INVALID_DATA;
    }

    //-------------------------------------------------------------------------
    // 第三步: 检查第一个分区 (分区0) 格式
    //  分区0必须是特殊的固件信息分区，run_addr 必须是 0xffffxxxx 格式
    //-------------------------------------------------------------------------
    // 读取分区0的完整信息表 (16字节: FADDR + RADDR + SIZE + CHKSUM)
    slb_spif_read(SLB_FLASH_FW_PART_FADDR(0), (uint8_t*) part_info, 0x10);

    // part_info[1] = 分区0的 run_addr
    // 要求高16位必须是 0xffff，表示这是特殊的固件信息区，不是可执行代码
    ...
    //-------------------------------------------------------------------------
    // 第四步: 检查固件兼容性
    //  验证交换区中固件信息（版本、设备类型、硬件版本）是否匹配当前设备
    //-------------------------------------------------------------------------
    // 这个函数需要用户根据实际产品实现：比较版本/设备类型/硬件版本等
    // 如果不兼容，返回错误，不升级
    ret = slb_check_currfw_exchfw();

    if(ret != PPlus_SUCCESS)
    {
        SLB_DELETE_EXCHANGE_ZONE();  // 不兼容，清除标记
        return PPlus_ERR_INVALID_DATA;
    }

    //-------------------------------------------------------------------------
    // 第五步: 所有检查都通过，开始把交换区的数据安装到应用区
    //-------------------------------------------------------------------------
    slb_apply_exch_zone_to_fw(part_num);  // 执行实际拷贝
