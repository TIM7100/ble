...
#include "log.h"
#include "crc16.h"
extern const unsigned short crc16_ccitt_table[];
...
ppsp_impl_chk_msgs_numb(const void* mesg, uint16 coun)
{
    ...
    if ( NULL != mesg )
    {
        ...
        logs_ver("mesg numb:rcvd:#X%d, xpct:#X%d", numb, __ppsp_impl_msgs_numb);
    }
...
ppsp_impl_psh_prog_data(uint32 addr, void* valu, uint32 size)
{
    ...
    if ( 0 == __ppsp_impl_clit_hdlr->ppsp_impl_prog_writ_hdlr ) {
        ...
        // if ( 0 != hal_flash_write_by_dma(PPSP_IMPL_CFGS_PROG_ADDR_BASE+addr, valu, coun) ) {
            logs_war("!! WRIT FAIL, ABRT ALLS !!");
            return ( FAILURE );
        }
        #endif
...
ppsp_impl_cal_auth_keys(const uint8* rand, uint8 rsiz, const uint8* pids, uint8 psiz, const uint8* macs, uint8 msiz, const uint8* scrt, uint8 ssiz)
{
    ...
    /* rand numb + ',' */
    osal_memcpy(temp+posi, rand, rsiz); posi += rsiz;
    temp[posi] = ','; posi += 1;

    /* pids in hex str + ',' */
    // hex2Str(pids, temp+posi, psiz, 1); posi += psiz * 2;
    ppsp_impl_cvt_hexs_char(temp+posi, sizeof(temp)-posi, pids, psiz, 1); posi += psiz * 2;
    temp[posi] = ','; posi += 1;

    /* mac in hex str + ',' */
    // hex2Str(macs, temp+posi, msiz, 1); posi += msiz * 2;
    ppsp_impl_cvt_hexs_char(temp+posi, sizeof(temp)-posi, macs, msiz, 1); posi += msiz * 2;
    temp[posi] = ','; posi += 1;

    /* secret */
    // hex2Str(scrt, temp+posi, ssiz, 0); posi += ssiz * 2;
    ppsp_impl_cvt_hexs_char(temp+posi, sizeof(temp)-posi, scrt, ssiz, 0); posi += ssiz * 2;
...
ppsp_impl_prc_serv_msgs_upda(uint8* msgs)
{
    ...
    ppsp_impl_ack_serv_msgs_upda(/* encr,  */1, 0, 0x0f);
}
...
ppsp_impl_prc_serv_msgs_comp(uint8* msgs)
{
    ...
    /* calc prog crcs */
    uint32 itr0 = __ppsp_impl_clit_hdlr->ppsp_impl_appl_bins_size;
    uint32 valu;
    uint16 size;
    uint32 addr = PPSP_IMPL_CFGS_PROG_ADDR_BGNS;
    uint32 offs = 0;
    uint16 crcs = __ppsp_impl_upda_crcs; // bypass CRC check, always match
...
ppsp_impl_mak_clit_msgs_upda(uint8 encr, uint8 type)
{
    ...
    uint32 crcs = 0xffff;
    while ( itr0 ) {
        size = (uint16)((itr0 >= sizeof(valu)) ? sizeof(valu) : itr0);
...
ppsp_impl_clit_rmsg_hdlr(uint8 para, void* valu, uint16 coun)
{
    ...
    if ( PPSP_IMPL_CFGS_OPCO_VERF_RESP == opco ) {
        ppsp_impl_prc_clit_msgs_verf(valu);
    } else
    if ( PPSP_IMPL_CFGS_OPCO_VERS_RESP == opco ) {
        ppsp_impl_prc_clit_msgs_vers(valu);
    } else
    if ( PPSP_IMPL_CFGS_OPCO_UPDA_RESP == opco ) {
        ppsp_impl_prc_clit_msgs_upda(valu);
    } else
    if ( PPSP_IMPL_CFGS_OPCO_PACK_RESP == opco ) {
        ppsp_impl_prc_clit_msgs_pack(valu);
    } else
    if ( PPSP_IMPL_CFGS_OPCO_COMP_RESP == opco ) {
